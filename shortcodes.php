...
function shortcodes_page(){
	...
	<div class="wrap">
		...
		<ol>
			...
			<li>[mos-product-carousel title="" amount="" height="" class=""] <span class="sdetagils">displays progress bar</span></li>	
		</ol>
	</div>
	<?php
}
...
function mos_product_carousel_func( $atts = array(), $content = '' ) {
	...
	$atts = shortcode_atts( array(
        ...
        'autoplay-speed' => 2000,
		'count' => 0,
        'type' => 'recent',//featured,sellers
	), $atts, 'mos-product-carousel' );
    
    $args = array(
        'post_type' => 'product',
        'posts_per_page'=>($atts['count'])?$atts['count']:'',
    );
    ...
    } elseif ($atts['type'] == 'sellers') {
        $args['meta_key'] = 'total_sales';
        $args['orderby'] = 'meta_value_num';
    }
    $size = (preg_match('/[0-9]+,[0-9]+/',$atts['size']))?$atts['size']:'370,370';
    $slice = explode(',',$size);
    $width = $slice[0];
    $height = $slice[1];

    $slidesToScroll = ($atts['grid'])?$atts['grid']:1;
    $slidesToScroll_992 = ($atts['grid-md'])?$atts['grid-md']:1;
    $slidesToScroll_786 = ($atts['grid-sm'])?$atts['grid-sm']:1;            
    $autoplay = ($atts['autoplay'])?$atts['autoplay']:true;
    $autoplaySpeed = ($atts['autoplay-speed'])?$atts['autoplay-speed']:2000;
    ...
    if ( $query->have_posts() ) :   
    ?>
        <div class="mos-product-wrapper <?php echo $atts['class'] ?>">
            <div class="slick-slider" data-slick='<?php echo $data_slick ?>'>
                <?php while ( $query->have_posts() ) : $query->the_post();?>
                    <?php global $product?>
                    <div class="item border h-100p">
                        <div class="wrapper">
                            <div class="img-part border-bottom">                                
                                <img src="<?php echo aq_resize(wp_get_attachment_url( $product->get_image_id() ),$width,$height,true) ?>" alt="<?php echo get_post_meta($product->get_image_id(), '_wp_attachment_image_alt', TRUE); ?>" class="img-fluid img-product-image">
                            ...
                            <div class="text-part text-center">
                                <a class="product-title" href="<?php echo get_the_permalink() ?>"><?php echo get_the_title() ?></a>
                                <?php echo $product->get_price_html(); ?>
                            </div> 
                        </div>
                        <div class="buy-part border-top p-5">
...
add_shortcode( 'mos-product-carousel', 'mos_product_carousel_func' );
...
function product_search_fnc( $atts = array(), $content = '' ) {
    ...
    <form role="search" method="get" class="woocommerce-product-search" action="<?php echo esc_url( home_url() )?>">
        <div class="input-group mb-3">
            <input type="text" class="form-control" placeholder="Search Product" value="<?php get_search_query()?>" name="s" class="form-control" required>
